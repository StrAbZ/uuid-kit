...
extension UUID {
    /// Creates a new time-based (version 6) UUID.
    ///
...
public struct UUIDv6: Codable, Hashable, LosslessStringConvertible, RawRepresentable {
    public init() {
    ...
    public init(timestamp: Timestamp, clockSequence: ClockSequence, node: Node) {
        ...
        assert(rawValue.version == 6)
    }

    public init?(rawValue: UUID) {
        guard rawValue.version == 6 else {
            return nil
        }
    ...
    public init?(_ description: String) {
        guard let rawValue = UUID(uuidString: description), rawValue.version == 6 else {
            return nil
        }
        self.rawValue = rawValue
    }
